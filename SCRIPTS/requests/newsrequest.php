...
if(isset($_POST["accessnewsrequest"])){
	...
	if(($_SESSION["permissions"][0] == 1 || $_SESSION["permissions"][1] == 1 || $_SESSION["permissions"][11] == 1 ) == false){
		echo "You do not have permission to modify News.";
		DBClose($conn);
		return;
	}
	...
	if($opcode == 1)
	{
		...
		if(isset($_POST["news_title"])){
			$news_title = mysqli_real_escape_string($conn, removeslashes($_POST["news_title"]));
		} else {
		...
		if(isset($_POST["news_text"])){
			$news_text = mysqli_real_escape_string($conn, removeslashes($_POST["news_text"]));
		} else {
		...
		$result = addNewsEntry($news_title, $news_text, $conn);
	...
	if($opcode == 3)
	{
		...
		if(isset($_POST["news_date"])){
			...
			$sets[] = "news_date = " . "str_to_date($news_date, '%m/%d/%Y')";
		}
